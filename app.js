...
const firebaseConfig = {
    ...
    databaseURL: "https://my2026-e8f99-default-rtdb.asia-southeast1.firebasedatabase.app",
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // 今天的樣式
        if (currentYear === today.year && currentMonth === today.month && day === today.day) {
    ...
    days.forEach(dayDiv => {
        ...
        if (dayItemsContainer) {
            ...
            for (let i = 0; i < displayCount; i++) {
                ...
                itemDiv.className = `day-item my-item${item.completed ? ' completed' : ''}${colorClass}`;
...
function openDailyModal(dateKey) {
    ...
    modalDate.textContent = `${year} 年 ${parseInt(month)} 月 ${parseInt(day)} 日`;
...
newItemColorPicker.querySelectorAll('.color-option').forEach(option => {
    option.addEventListener('click', (e) => {
        newItemColorPicker.querySelectorAll('.color-option').forEach(o => o.classList.remove('selected'));
...
editItemColorPicker.querySelectorAll('.color-option').forEach(option => {
    option.addEventListener('click', (e) => {
        editItemColorPicker.querySelectorAll('.color-option').forEach(o => o.classList.remove('selected'));
        e.target.classList.add('selected');
...
addAndApplyBtn.addEventListener('click', async () => {
    ...
    const confirmApply = confirm(`確定要將「${text}」新增到 ${currentYear} 年 ${currentMonth + 1} 月的所有日期嗎？`);
    ...
    try {
        ...
        newItemInput.value = '';
        alert(`已成功將「${text}」新增到 ${currentMonth + 1} 月全部 ${daysInMonth} 天！`);
    } catch (error) {
...
async function handleDayDrop(e) {
    ...
    const hasDuplicate = Object.values(targetItems).some(item => item.text === draggedItem.text);

    if (hasDuplicate) {
        const confirmMove = confirm(`「${draggedItem.text}」在 ${formatDateDisplay(targetDate)} 已存在，是否仍要移動？\n\n（會產生重複項目）`);
        if (!confirmMove) {
            draggedItem = null;
            return;
        }
    ...
    try {
        ...
        // 從原日期刪除項目
        const oldItemRef = ref(db, `users/${currentUser}/dailyGoals/${draggedItem.sourceDate}/items/${draggedItem.itemId}`);
...
saveEditBtn.addEventListener('click', async () => {
    ...
    try {
        const itemRef = ref(db, `users/${currentUser}/dailyGoals/${selectedDate}/items/${editingItemId}`);
...
async function loadWeekGoal() {
    ...
    const weekGoalRef = ref(db, `users/${currentUser}/weeklyGoals/${currentWeekKey}/items`);
...
function renderWeekGoalsList() {
    ...
    weekGoalsList.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        cb.addEventListener('change', (e) => toggleWeekGoalItem(e.target.dataset.id, e.target.checked));
    });
    weekGoalsList.querySelectorAll('.goal-item-delete').forEach(btn => {
        btn.addEventListener('click', (e) => deleteWeekGoalItem(e.target.dataset.id));
    });
}
...
async function addWeekGoalItem() {
    ...
    const itemId = Date.now().toString();
    const itemRef = ref(db, `users/${currentUser}/weeklyGoals/${currentWeekKey}/items/${itemId}`);
...
function calculateProgressRate() {
    ...
    const maxDay = isCurrentMonth ? today.day :
        (currentYear < today.year || (currentYear === today.year && currentMonth < today.month)) ?
            new Date(currentYear, currentMonth + 1, 0).getDate() : 0;
    ...
    if (currentUser && isCurrentMonth) {
        const monthKey = getMonthKey(currentYear, currentMonth);
        const monthGoalRef = ref(db, `users/${currentUser}/monthlyGoals/${monthKey}`);
